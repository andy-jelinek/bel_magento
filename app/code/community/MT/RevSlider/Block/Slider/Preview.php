...
class MT_RevSlider_Block_Slider_Preview extends Mage_Core_Block_Template implements Mage_Widget_Block_Interface{
        ...
        if ($slider->getId() && $slider->getStatus() == 1){
            ...
            if ($slider->getData('show_background_image') == 'true'){
                if ($backgroundImage = $slider->getData('background_image')){
                    $bannerStyle .= "background-image:url({$backgroundImage});background-repeat:no-repeat;background-size:100%;background-position:center;";
                }
            }
    ...
    public function renderSlides($slider){
        ...
        if ($slider && $this->numSlides){
            $thumbWidth = $slider->getData('thumb_width');
            $thumbHeight = $slider->getData('thumb_height');
            //$slideW = $slider->getData('width');
            //$slideH = $slider->getData('height');
            ...
            foreach ($slides as $slide){
                ...
                if ($slide->getData('background_type') != 'external'){
                ...
                $output .= "<img src='{$urlSlideImage}' {$styleImage} {$imageAddParams}/>";
    ...
    public function renderLayers($slide){
        ...
        foreach ($slide->getLayers() as $layer){
            ...
            $htmlParams = $htmlEnd.$htmlLink.$htmlVideo.$htmlHidden;
    ...
    public function renderJs($slider){
        ...
        jQuery(document).ready(function(){
            jQuery('#{$this->sliderHtmlId}').show().revolution({
                ...
                navigationHAlign: '{$slider->getData("navigation_align_hor")}',
                navigationVAlign: '{$slider->getData("navigation_align_vert")}',
                navigationHOffset: '{$slider->getData("navigation_offset_hor")}',
                navigationVOffset: '{$slider->getData("navigation_offset_vert")}',

                soloArrowLeftHalign: '{$slider->getData("leftarrow_align_hor")}',
                soloArrowLeftValign: '{$slider->getData("leftarrow_align_vert")}',
                soloArrowLeftHOffset: '{$slider->getData("leftarrow_offset_hor")}',
                soloArrowLeftVOffset: '{$slider->getData("leftarrow_offset_vert")}',

                soloArrowRightHalign: '{$slider->getData("rightarrow_align_hor")}',
                soloArrowRightValign: '{$slider->getData("rightarrow_align_vert")}',
                soloArrowRightHOffset: '{$slider->getData("rightarrow_offset_hor")}',
                soloArrowRightVOffset: '{$slider->getData("rightarrow_offset_vert")}',
