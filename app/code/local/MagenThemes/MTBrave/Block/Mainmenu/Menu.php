...
?>
<?php
class MagenThemes_MTBrave_Block_Mainmenu_Menu extends Mage_Catalog_Block_Navigation
{
    protected $_column;
                                                   ...
                                                   $isOutermost = false, $outermostItemClass = '', $childrenWrapClass = '', $noEventAttributes = false)
    {
        ...
        if (Mage::helper('mtbrave')->getCfg('menu/show_if_no_children')) {$showblock = true; }
    ...
    protected function _getCategoryLabelHtml($category, $level){
        ...
        if ($labelCategory){
            $getLabel = trim(Mage::helper('mtbrave')->getCfg('menu/' . $labelCategory));
            if ($getLabel) {
                if ($level == 0){
