...
?>
<?php
class MagenThemes_MTBrave_Model_Cssgen_Generator extends Mage_Core_Model_Abstract{
    ...
    protected function _generateStoreCss($design, $storeCode){
        ...
        $filedefault = Mage::helper('mtbrave/cssgen')->getGeneratedCssDir() . $filename;
        $path = 'magenthemes/mtbrave/css/' . $design . '.phtml';
        ...
        try{
            ...
            if (empty($block)) {
                throw new Exception( Mage::helper('mtbrave')->__("Template file is empty or doesn't exist: %s", $path) );
            }
            $file = new Varien_Io_File();
            $file->setAllowCreateFolders(true);
            $file->open(array( 'path' => Mage::helper('mtbrave/cssgen')->getGeneratedCssDir() ));
            ...
            Mage::getSingleton('adminhtml/session')->addError(Mage::helper('mtbrave')->__('Failed generating CSS file: %s in %s', $filename, Mage::helper('mtbrave/cssgen')->getGeneratedCssDir()). '<br/>Message: ' . $gener->getMessage());
